...
int main()
{
    ReadStudentsFromFile();  // Read existing data from the file
    ...
    while (IsRunning)
    {
        ...
        case 0:
            IsRunning = false;
            WriteStudentsToFile();  // Write data to the file before exiting
            break;
        case 1:
              system("cls");
            printf("\n\t\t **** Add A New Student ****\n\n");
...
void WriteStudentsToFile()
{
    ...
    for (int i = 0; i < TotalStudents; i++)
    {
        fprintf(file, "%s %s %s %s %d\n",
                ...
                Students[i].Phone, Students[i].NumberOfCourse);
    }
...
void ReadStudentsFromFile()
{
    ...
    while (fscanf(file, "%s %s %s %s %d",
                  Students[TotalStudents].ID, Students[TotalStudents].Name,
